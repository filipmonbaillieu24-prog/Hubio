...
export function useTrainingState(
  ...
  const pmcStatus = useMemo(() => {
    const tssList = rides.filter(r => (r.tss ?? r.hrTSS) != null).map(r => ({ date: r.date, tss: (r.tss ?? r.hrTSS)! }));
  ...
  const recommendedType = useMemo(() => {
    ...
    const predicted = predictRecommendedWorkout(
      pmcStatus.ctl,
      pmcStatus.atl,
      pmcStatus.tsb,
  ...
  }, [pmcStatus, profile.trainingGoal, avgRpeLast3, trainingProfile.isFlexible, flexibleRecommendation.type, phaseInfo.phase]);
  ...
  // ── Acute Chronic ratio ──
  const acuteChronicRatio = pmcData.ctl > 0 ? (pmcData.atl / pmcData.ctl) : 0;
  const overtrainingRisk = (acuteChronicRatio > 1.5 ? 'hoog' : acuteChronicRatio > 1.3 ? 'matig' : null) as 'hoog' | 'matig' | null;
