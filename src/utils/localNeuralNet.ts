...
class SimpleMLP {
  ...
  constructor(
    ...
    try {
      ...
      console.error(`Kon gewichten voor ${storageKey} niet laden:`, e);
    }

    if (loaded && loaded.W1 && loaded.B1 && loaded.W2 && loaded.B2 && 
        loaded.W1.length === inputSize && loaded.B1.length === hiddenSize) {
      this.W1 = loaded.W1;
      this.B1 = loaded.B1;
      this.W2 = loaded.W2;
      this.B2 = loaded.B2;
  ...
  save() {
    try {
      ...
      console.error(`Kon gewichten voor ${this.storageKey} niet opslaan:`, e);
    }
  }
  ...
  train(x: number[], targets: number[], lr: number = 0.15): number[] {
    ...
    const yIn = new Array(this.B2.length).fill(0);
    const y = new Array(this.B2.length).fill(0);
    for (let col = 0; col < this.B2.length; col++) {
      let sum = 0;
      for (let row = 0; row < h.length; row++) {
      ...
      yIn[col] = sum + this.B2[col];
      y[col] = 1 / (1 + Math.exp(-yIn[col])); // sigmoid
    }
...
export function trainOnCorrection(text: string, corrected: NeuralAnalysis, learningRate: number = 0.15): NeuralAnalysis {
  ...
  const y = notesModel.train(x, targets, learningRate);
  return {
    fatigue: parseFloat(y[0].toFixed(2)),
    recovery: parseFloat(y[1].toFixed(2)),
    illness: parseFloat(y[2].toFixed(2))
...
export function predictRecommendedWorkout(
  ...
  const x = [
    ...
    Math.max(0, Math.min(1, (tsb + 50) / 100)), // scale TSB -50..+50 to 0..1
    goalIndex === 0 ? 1 : 0,
    goalIndex === 1 ? 1 : 0,
    goalIndex === 2 ? 1 : 0,
    goalIndex === 3 ? 1 : 0,
...
export function trainCoachModel(
  ...
  goal: string,
  avgRpeLast3: number,
  selectedWorkout: typeof COACH_WORKOUTS[number]
): void {
  const goalIndex = ['general', 'climbing', 'speed', 'endurance'].indexOf(goal);
  const x = [
    Math.min(1.5, ctl / 100),
  ...
  const targetIdx = COACH_WORKOUTS.indexOf(selectedWorkout);
  if (targetIdx === -1) return;

  const targets = new Array(6).fill(0.05); // low probability default
  targets[targetIdx] = 0.95; // high probability target

  coachModel.train(x, targets, 0.2); // train network
}
...
export function predictRPE(
  ...
  const x = [
    Math.min(1.0, duration / 36000), // scale 10 hours
    Math.min(1.0, distance / 200),   // scale 200km
    Math.min(1.5, intensityFactor),
    Math.min(1.0, tss / 500),
    Math.max(0, Math.min(0.5, variabilityIndex - 1.0)),
    Math.min(1.0, (avgHR || 130) / 220)
  ];

  const y = rpeModel.predict(x);
  // Map output 0..1 to RPE 1..10
  const score = Math.max(1, Math.min(10, Math.round(1 + y[0] * 9)));
  return score;
}
...
export function trainRPEModel(
  ...
  const x = [
    Math.min(1.0, duration / 36000),
    Math.min(1.0, distance / 200),
    Math.min(1.5, intensityFactor),
    Math.min(1.0, tss / 500),
    Math.max(0, Math.min(0.5, variabilityIndex - 1.0)),
    Math.min(1.0, (avgHR || 130) / 220)
  ...
  const target = (actualRpe - 1) / 9; // scale 1..10 to 0..1
  rpeModel.train(x, [target], 0.2);
}
...
export function trainLabelModel(
  ...
  hasPower: boolean,
  avgHR: number,
  actualLabel: typeof RIDE_LABELS_KEYS[number]
): void {
  const x = [
    Math.min(1.5, intensityFactor),
    Math.max(0, Math.min(0.5, variabilityIndex - 1.0)),
  ...
  const targetIdx = RIDE_LABELS_KEYS.indexOf(actualLabel);
  if (targetIdx === -1) return;

  const targets = new Array(7).fill(0.05);
  targets[targetIdx] = 0.95;

  labelModel.train(x, targets, 0.25);
}

// ─── MODEL 5: eFTP & PROGRESSIE VOORSPELLER ─────────────────────────────────────

function generateFTPDefaultWeights() {
  // Input: [currentFtp/500, ctl/100, atl/100, consistency/7, avgTssLast30/100] (5 features)
  // Hidden: 6, Output: 1
...
const ftpModel = new SimpleMLP(5, 6, 1, 'cyclo_ftp_nn_weights', generateFTPDefaultWeights);
...
export function predictFutureFTP(
  ...
  const x = [
    ...
    Math.min(1.5, ctl / 100),
    Math.min(1.5, atl / 100),
    Math.min(1.0, consistency / 7),
    Math.min(2.0, avgTssLast30 / 100)
  ];

  const y = ftpModel.predict(x);
  // Map output 0..1 to multiplier 0.95..1.15 (predicted FTP progress)
  const multiplier = 0.95 + y[0] * 0.20;
...
export function trainFTPModel(
  ...
  const multiplier = actualFtpIn8Weeks / (currentFtp || 220);
  const target = (multiplier - 0.95) / 0.20; // scale to 0..1
  ftpModel.train(x, [Math.max(0, Math.min(1, target))], 0.15);
}

// ─── MODEL 6: OVERTRAINING & INJURY RISK ────────────────────────────────────────

function generateInjuryDefaultWeights() {
  // Input: [CTL/100, ATL/100, TSB/100, fatigueScore, illnessScore, acuteChronicRatio/3] (6 features)
  // Hidden: 6, Output: 1
  const W1: number[][] = Array.from({ length: 6 }, () => new Array(6).fill(0));
  const B1: number[] = new Array(6).fill(0.0);
  const W2: number[][] = Array.from({ length: 6 }, () => new Array(1).fill(0));
...
const injuryModel = new SimpleMLP(6, 6, 1, 'cyclo_injury_nn_weights', generateInjuryDefaultWeights);
...
export function trainInjuryModel(
  ...
  injuryModel.train(x, [actualInjuryOccurred ? 0.95 : 0.05], 0.15);
}

// ─── MODEL 7: CLIMB TIME PREDICTOR ─────────────────────────────────────────────

function generateClimbDefaultWeights() {
  // Input: [lengthMeters/10000, gradePct/25, ftp/500, weightKg/150] (4 features)
  // Hidden: 6, Output: 1
  ...
  for (let i = 0; i < 6; i++) {
    W1[0][i] = 0.8;  // length
    W1[1][i] = 0.9;  // grade
    W1[2][i] = -0.5; // ftp (higher FTP decreases climb time)
    W1[3][i] = 0.4;  // weight (higher weight increases climb time)
    W2[i][0] = 0.6;
  }
...
const climbModel = new SimpleMLP(4, 6, 1, 'cyclo_climb_nn_weights', generateClimbDefaultWeights);
...
export function predictClimbTime(
  ...
  const x = [
    Math.min(1.0, lengthMeters / 10000),
    Math.min(1.0, gradePct / 25),
    Math.min(1.5, (ftp || 220) / 500),
    Math.min(1.0, (weightKg || 75) / 150)
  ];

  const y = climbModel.predict(x);
  // Scale output 0..1 to time in seconds (max 7200s, i.e. 2 hours)
  // Let's make sure a baseline is always calculated (at least length / average speed 15km/h)
  const baselineSpeed = (15 + (ftp / (weightKg || 75)) * 2) / 3.6; // m/s
  const minTime = lengthMeters / baselineSpeed;
  const predicted = y[0] * 7200;
  return Math.round(Math.max(minTime, predicted));
}
...
export function trainClimbModel(
  ...
  const target = Math.max(0, Math.min(1, actualTimeSeconds / 7200));
  climbModel.train(x, [target], 0.2);
}
...
export function trainOptimalCadence(power: number, bestCadence: number): void {
  ...
  cadenceModel.train(x, [target], 0.15);
}
...
export function trainRouteDurationModel(
  ...
  routeDurationModel.train(x, [target], 0.15);
}
...
export function trainVO2maxModel(
  ...
  vo2maxModel.train(x, [target], 0.15);
}
