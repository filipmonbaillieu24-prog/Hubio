...
export function generateCoachAdvice(
  ...
  if (prevWeekTSS > 20) {
    ...
    if (growth > 15) {
      advice.push({
        category: 'waarschuwing', priority: 1, icon: '⚠️', color: '#ff7675',
        ...
        action: 'Plan morgen een herstelrit of rustdag',
      });
    } else if (growth < -35 && weekTSS < 40) {
      advice.push({
        category: 'training', priority: 2, icon: '💤', color: '#74b9ff',
        title: 'Lage trainingsbelasting deze week',
        body:  `Je TSS is ${Math.round(weekTSS)} vs ${Math.round(prevWeekTSS)} vorige week. Als dit geen bewuste rust is, probeer een zone 2 rit van 60–90 minuten.`,
        action: 'Plan een duurrit van 60–90 min',
      });
    }
  }
  ...
  if (totalTime > 7200 && z2Time / totalTime < 0.55) {
    advice.push({
      ...
      action: '2–3 ritten per week van 60+ min op matige intensiteit',
    });
  }
  ...
  if (!last.hasPower) {
    ...
    if (efRides.length >= 4) {
      ...
      if (efPct > 2) {
        advice.push({
          category: 'progressie', priority: 3, icon: '📈', color: 'var(--color-accent,#39ff14)',
        ...
        advice.push({
          category: 'herstel', priority: 2, icon: '😴', color: '#fdcb6e',
  ...
  if (last.hasPower) {
    ...
    if (ftpRides.length >= 5) {
      ...
      if (ftpGrow > 3) {
        advice.push({
          category: 'progressie', priority: 3, icon: '⚡', color: 'var(--color-primary,#00e5ff)',
        ...
        advice.push({
          category: 'waarschuwing', priority: 2, icon: '📉', color: '#ff7675',
          title: 'eFTP daalt',
  ...
  if (daysSinceLast > 6 && recent.length < 3) {
    advice.push({
      ...
      body:  `Je laatste rit was ${Math.floor(daysSinceLast)} dagen geleden. Zelfs een korte rit van 30 min houdt je basisconditie op peil.`,
      action: 'Rijd vandaag minstens 30 minuten',
    });
  }
  ...
  if (highDrift.length >= 2) {
    advice.push({
      category: 'herstel', priority: 2, icon: '❤️‍🔥', color: '#ff7675',
      title: 'Hoge cardiac drift recent',
      body:  `${highDrift.length} recente ritten met >8% cardiac drift. Dit wijst op accumulerende vermoeidheid, ondervulling of ziekte.`,
  ...
  if (profile.weight && last.eFTP) {
    ...
    if (wpkg < 2.0) {
      advice.push({
        category: 'doel', priority: 3, icon: '🎯', color: '#a29bfe',
      ...
      advice.push({
        category: 'doel', priority: 3, icon: '🎯', color: 'var(--color-accent,#39ff14)',
        title: `Goed niveau: ${wpkg.toFixed(2)} W/kg`,
        body:  'Je bent op weg naar gevorderd niveau (3.0+ W/kg). Combineer lange duurritten met 1–2 intervaltrainingen per week (sweet spot / threshold).',
      });
    } else {
      advice.push({
        category: 'progressie', priority: 3, icon: '🏆', color: 'var(--color-primary,#00e5ff)',
  ...
  if (cadRides.length >= 3) {
    ...
    if (avgCad < 80) {
      advice.push({
        ...
        body:  'Je gemiddelde cadans is onder 80 rpm. Hogere cadans (85–95 rpm) vermindert spierschade en verhoogt duurprestatie. Oefen bewust met een hogere cadans op makkelijke ritten.',
        action: 'Oefen 85–95 rpm op je volgende rit',
      });
    }
  }
  ...
  if (totalTime > 7200 && z3Time / totalTime > 0.25) {
    advice.push({
      category: 'waarschuwing', priority: 2, icon: '⚠️', color: '#ff9f43',
      ...
      action: 'Houd je duurritten strikt in Zone 2 en intensieve trainingen in Zone 4+'
    });
  }

  // ── 10. W' Anaerobe tank depletion ──────────────────────────────────────────
  // Check of de atleet recent zijn batterij diep heeft leeggereden (sprint/interval ritten)
  const powerRides = recent.filter(r => r.hasPower && r.bestEfforts);
  if (powerRides.length > 0) {
    // Zoek naar ritten met zeer hoge 5s / 1 min vermogens in verhouding tot FTP
    ...
    if (deepDepletionRides.length >= 2) {
      advice.push({
        category: 'progressie', priority: 3, icon: '🔋', color: '#39ff14',
        title: 'Diepe anaerobe W\' uitputting gemeten',
        body: `In ${deepDepletionRides.length} van je recente ritten heb je je anaerobe tank ($W'$) diep aangesproken door explosieve inspanningen boven je drempel. Dit stimuleert de glycolytische capaciteit en je sprintcapaciteit.`,
        action: 'Zorg voor minimaal 48u herstel na ritten met dergelijke explosieve prikkels'
      });
    }
  }
  ...
  if (goal === 'climbing') {
    advice.push({
      category: 'doel', priority: 2, icon: '⛰️', color: '#fdcb6e',
    ...
    advice.push({
      category: 'doel', priority: 2, icon: '⚡', color: '#00e5ff',
    ...
    advice.push({
      category: 'doel', priority: 2, icon: '🚴', color: '#00b894',
      title: 'Focus: Lange duurritten (Endurance)',
      body: 'Je doel is vetverbranding en uithoudingsvermogen. Richt je op het vergroten van je aerobe basis door minstens één lange rit te plannen die volledig in Zone 2 (Uithoudingsvermogen) valt.',
      action: 'Plan een rustige duurrit van minimaal 2.5 tot 3 uur in Zone 2'
    });
  }
  ...
  if (pmcData && pmcData.ctl > 0) {
    ...
    if (tsb < -30) {
      advice.push({
        category: 'waarschuwing', priority: 1, icon: '🔴', color: '#ff7675',
        ...
        action: '2–3 volledige rustdagen of een lichte herstelrit (Zone 1)',
      });
    } else if (tsb < -20) {
      advice.push({
        category: 'herstel', priority: 2, icon: '⚡', color: '#fdcb6e',
        ...
        action: 'Prioriteer 7–8u slaap, voldoende koolhydraten en hydratatie',
      });
    } else if (tsb > 10 && tsb < 25) {
      advice.push({
        category: 'progressie', priority: 3, icon: '🏆', color: '#55efc4',
        ...
        action: 'Plan een tijdrit of race in de komende 3–5 dagen',
      });
    } else if (tsb > 25) {
      advice.push({
        category: 'training', priority: 2, icon: '😴', color: '#74b9ff',
        ...
        action: 'Herneem geleidelijk met een endurance- of sweetspot-rit',
      });
    }
    ...
    if (ctl < 20 && rides.length >= 5) {
      advice.push({
        ...
        action: 'Voeg één extra duurrit per week toe van 60–90 min in Zone 2',
      });
    } else if (ctl > 70) {
      advice.push({
        category: 'progressie', priority: 3, icon: '🔥', color: '#00e5ff',
    ...
    if (atl > ctl * 1.5 && ctl > 10) {
      advice.push({
        category: 'waarschuwing', priority: 1, icon: '⚠️', color: '#ff9f43',
