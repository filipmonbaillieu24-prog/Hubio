...
# Define paths
$GradleFile = "apps/zenith-pilot/app/build.gradle.kts"
$ApkSrcPath = "apps/zenith-pilot/app/build/outputs/apk/debug/app-debug.apk"
$ApkDestPath = "apk/app-debug.apk"
$VersionJsonPath = "apk/version.json"
...
Push-Location "apps/zenith-pilot"
try {
...
$HubApkPath = "apps/zenith-hub/public/app-debug.apk"
...
Write-Host "Updated version.json at $VersionJsonPath." -ForegroundColor Green
...
git commit -m "Release v$NewVersionName (versionCode $NewVersionCode)"

Write-Host "Pushing changes to origin main..." -ForegroundColor Yellow
git push origin main

Write-Host "Zenith Pilot update v$NewVersionName published successfully!" -ForegroundColor Green
